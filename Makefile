...
FILE_SUFFIX=$(PERFORMANCE_TESTING_SUFFIX)$(GUIDS_WITHOUT_MINGW_SUFFIX)$(WITHOUT_RUNTIME_SUFFIX)$(WITHOUT_CRITICAL_SECTIONS_SUFFIX)$(UNICODE_SUFFIX)

ALL_OBJECT_FILES=$(OBJ_DIR)\DisplayError$(FILE_SUFFIX).o $(OBJ_DIR)\EntryPoint$(FILE_SUFFIX).o $(OBJ_DIR)\MainFormWndProc$(FILE_SUFFIX).o $(OBJ_DIR)\WinMain$(FILE_SUFFIX).o $(OBJ_DIR)\Resources$(FILE_SUFFIX).obj
...
.PHONY: all clean install uninstall configure IrcChat

IrcChat: $(BIN_DIR)\IrcChat.exe

$(BIN_DIR)\IrcChat.exe: $(ALL_OBJECT_FILES)
	$(GCC_LINKER) -m $(PE_FILE_FORMAT) -subsystem console -e $(ENTRY_POINT) $(DEFAULT_STACK_SIZE) $(LINKER_STRIP_FLAG) -L $(COMPILER_LIB_PATH) -L "." $(FBEXTRA) $(ALL_OBJECT_FILES) -( $(ALL_OBJECT_LIBRARIES) -) -o "$(BIN_DIR)\IrcChat.exe"


$(OBJ_DIR)\WinMain$(FILE_SUFFIX).o: $(OBJ_DIR)\WinMain$(FILE_SUFFIX).asm
	$(GCC_ASSEMBLER) $(TARGET_ASSEMBLER_ARCH) $(ASSEMBLER_STRIP_FLAG) $(OBJ_DIR)\WinMain$(FILE_SUFFIX).asm -o $(OBJ_DIR)\WinMain$(FILE_SUFFIX).o

$(OBJ_DIR)\WinMain$(FILE_SUFFIX).asm: $(OBJ_DIR)\WinMain$(FILE_SUFFIX).c
	$(GCC_COMPILER) $(GCC_COMPILER_PARAMETERS) $(OBJ_DIR)\WinMain$(FILE_SUFFIX).c -o $(OBJ_DIR)\WinMain$(FILE_SUFFIX).asm

$(OBJ_DIR)\WinMain$(FILE_SUFFIX).c: Modules\WinMain.bas
	$(FREEBASIC_COMPILER) $(FREEBASIC_PARAMETERS) "Modules\WinMain.bas"
	move /y Modules\WinMain.c $(OBJ_DIR)\WinMain$(FILE_SUFFIX).c


$(OBJ_DIR)\DisplayError$(FILE_SUFFIX).o: $(OBJ_DIR)\DisplayError$(FILE_SUFFIX).asm
	$(GCC_ASSEMBLER) $(TARGET_ASSEMBLER_ARCH) $(ASSEMBLER_STRIP_FLAG) $(OBJ_DIR)\DisplayError$(FILE_SUFFIX).asm -o $(OBJ_DIR)\DisplayError$(FILE_SUFFIX).o

$(OBJ_DIR)\DisplayError$(FILE_SUFFIX).asm: $(OBJ_DIR)\DisplayError$(FILE_SUFFIX).c
	$(GCC_COMPILER) $(GCC_COMPILER_PARAMETERS) $(OBJ_DIR)\DisplayError$(FILE_SUFFIX).c -o $(OBJ_DIR)\DisplayError$(FILE_SUFFIX).asm

$(OBJ_DIR)\DisplayError$(FILE_SUFFIX).c: Modules\DisplayError.bas
	$(FREEBASIC_COMPILER) $(FREEBASIC_PARAMETERS) "Modules\DisplayError.bas"
	move /y Modules\DisplayError.c $(OBJ_DIR)\DisplayError$(FILE_SUFFIX).c
...
$(OBJ_DIR)\MainFormWndProc$(FILE_SUFFIX).o: $(OBJ_DIR)\MainFormWndProc$(FILE_SUFFIX).asm
	$(GCC_ASSEMBLER) $(TARGET_ASSEMBLER_ARCH) $(ASSEMBLER_STRIP_FLAG) $(OBJ_DIR)\MainFormWndProc$(FILE_SUFFIX).asm -o $(OBJ_DIR)\MainFormWndProc$(FILE_SUFFIX).o

$(OBJ_DIR)\MainFormWndProc$(FILE_SUFFIX).asm: $(OBJ_DIR)\MainFormWndProc$(FILE_SUFFIX).c
	$(GCC_COMPILER) $(GCC_COMPILER_PARAMETERS) $(OBJ_DIR)\MainFormWndProc$(FILE_SUFFIX).c -o $(OBJ_DIR)\MainFormWndProc$(FILE_SUFFIX).asm

$(OBJ_DIR)\MainFormWndProc$(FILE_SUFFIX).c: Modules\MainFormWndProc.bas
	$(FREEBASIC_COMPILER) $(FREEBASIC_PARAMETERS) "Modules\MainFormWndProc.bas"
	move /y Modules\MainFormWndProc.c $(OBJ_DIR)\MainFormWndProc$(FILE_SUFFIX).c
